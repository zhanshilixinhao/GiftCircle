...
import com.chouchongkeji.goexplore.pay.weixin.common.SPConfig;
...
public class SPPayService {
    ...
    private SPPayService(PayVO payVO) {
        this.payVo = payVO;
    }

    public static SPPayService service(PayVO payVO) {
        return new SPPayService(payVO);
    }
...
//        entity = "<xml><appid>wx67a01a97bda59412</appid><body>珠宝商城-商品购买</body><mch_id>1465159902</mch_id><nonce_str>217F5E7754C92D28FC6835D42F43548D</nonce_str><notify_url>http://localhost:8080/order/wx</notify_url><out_trade_no>10038</out_trade_no><spbill_create_ip>127.0.0.1</spbill_create_ip><total_fee>100</total_fee><trade_type>APP</trade_type><sign>08F937F558E757C78404887A49C45990</sign></xml>";
    ...
    private void assembleAppPayParmater(WXPayDto payDto) {
        // 微信app id
        payDto.setAppid(SPConfig.getAppid());
        // 微信商户号
        payDto.setPartnerid(SPConfig.getMchid());
        // 随机字符串
...
//        nonceStr = "B2004314AA49D95302179246148E0326";
        ...
        // appid
        packageParams.add(new NameValueModel("appid", SPConfig.getAppid()));
        // 商品描述
        packageParams.add(new NameValueModel("body", String.format("%s%s", payVo.getBody(), payVo.getSubject())));
        // 商户号
        packageParams.add(new NameValueModel("mch_id", SPConfig.getMchid()));
        // 随机字符串，不长于32位
...
//            Log.e(TAG, "genProductArgs: 总金额" + payMessage.price * 100);
        // 交易类型 此处为app
        packageParams.add(new NameValueModel("trade_type", "JSAPI"));
        // 签名
    ...
    private String genNonceStr() {
        ...
        return WXPayService.MD5.getMessageDigest(String.valueOf(random.nextInt(10000))).toUpperCase();
    }
