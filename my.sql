...
CREATE TABLE store_turnover (
   id INT ( 10 ) NOT NULL auto_increment,
   store_member_id int(11) comment'门店金额详情表Id（store_member_charge）（此次消费,type=2 门店金额详情表(活动卡)id(store_member_event)）',
   blag_money decimal(18,2) comment'索要金额（本金，收入）',
   turnover_money decimal(18,2) comment'营业额(赠送)',
   store_id int(11) comment'消费门店id(使用门店)',
   blag_store_id int(11) comment'索要门店id(索要消费金额,之前充值的门店)',
   event_id int(11) comment'会员卡活动id',
   store_charge_id int(11) comment'门店金额详情表Id（从哪次充值里扣款,type=2 门店金额详情表(活动卡)id(store_member_event)）',
   store_send_id int(11) comment'门店金额详情表Id（从哪次充值得到的赠送里扣款,type=2 门店金额详情表(活动卡)id(store_member_event)）',
   `type` tinyint(4) comment' 1 普通储值卡消费 2 活动卡消费 ',
   updated datetime COMMENT '修改时间',
   created datetime COMMENT '创建时间',
   PRIMARY KEY ( id ),
  key store_member_id(store_member_id),
...
CREATE TABLE card_rebate (
  ...
  record_id int(11) comment'充值/消费记录id',
