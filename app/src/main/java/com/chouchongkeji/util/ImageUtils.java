...
//        System.out.println(base64);
        ...
        String imgUri = module.trim() +
                "/" +
                DateUtil.today("yyyyMMdd") + "/";

        /** 生成图片磁盘全路径 **/
        String ImgPath = PropertiesUtil.imgSavePath() + imgUri;
        ...
        /** 图片写入磁盘 **/
        ImageIO.write(thumbnail, suffix, new File(ImgPath + fileName));
        /** 返回数据库存储路径 **/
    ...
    public static void delImage(String uri) {
        String path = PropertiesUtil.imgSavePath();
        if (path == null) return;
