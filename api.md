...
| type |  int  |    是    |   无   | 验证码类型 1 绑定手机号 2 重置赠送密码 3 修改用户号码，4-小程序提货验证手机 5-小程序管理端退款 6-会员卡找回密码 |
...

{
  ...
  "data": [
    {
      "itemId": 601, //商品id
      "cover": "https://liyuquan.cn/static/item/190409/b99b82b5-0805-48c8-897a-41c8608ee665.jpg", // 商品封面
      "title": "彩云之荞苦荞茶胚芽茶120g", //商品标题
      "brandName": "土大姐", //品牌名称
      "day": 1561327812000 //展示日期（当日）
    },
    {
      "itemId": 602,
      "cover": "https://liyuquan.cn/static/item/190409/f0b74198-7711-4c63-a075-e1d7f56b373f.jpg",
      "title": "200g寻花茉莉花花草调味茶花饼",
      "brandName": "土大姐",
      "day": 1561327812000
    }
  ]
...
| payWay | int | 是 | 无 | 支付方式，微信 24656 ，支付宝 78990，余额98001 ，87661 礼花,36666 礼遇圈会员卡|
...
| :----------: | :------: | :------: | :----: | :------: |
| access_token |  string  |    是    |   无   | 被邀请者token |
|phone|String|是|无|电话号码（多个时用逗号隔开）|
...
### 12.10 用户未查看的秀秀评论,点赞和未查看的背包物品数量
  ...
  "data": [
    {
      "type": 1,  // 1 商品 2 虚拟物品
      "ratio": 0.2, //折现比例
      "desc": "平台收取商品原价的20%作为仓储及物流服务费"  //描述
    },
    {
      "type": 2,
      "ratio": 0,
...
### 14.11 背包未查看数量(我的背包红点)

- 请求地址：auth/v1/bp/unread
- 服务协议：HTTP/POST
- 是否需要身份认证：是
- 作者：linqin
...
- 请求地址：auth/v1/gift/getGiftStatus
- 服务协议：HTTP/POST
- 是否需要身份认证：是
- 作者：yichen
...
```js
{
  ...
  data: {
        type: 3,
         "status": 2 // 0 可以领取，1 我已领取 2 我未领取但礼物已被领完 3 我是我送的我自己不能领取 4领取过一次并且没有领取到
        giftInfo:  {
            recordDetailId: 2,  // 礼物赠送记录详情id，可用于答谢
            userId: 4, 
            greetting: "一厢情愿的不舍", // 祝福语
...
```js
{
  ...
  "data": {
    ...
    "quantity": 10, //总数
    "targetDate": 1550673648000, //福利发放时间(福利可以领取时间)
...
```js
{
  ...
  "data": {
    ...
    "title": "50", //标题
    "brand": "50" //品牌
    isReceive: 1 //是否领取 1 未领取 2已领取
    "remainTime":99031  //领取时间倒计时(毫秒)
  }
}
...
```js
{
  ...
  data: {
        type: 1// 1 商品 2 虚拟商品,
         "status": 2 // 0 正常领取  1 已领取 2 礼物没有了,被领完了
        giftInfo:  {
            recordDetailId: 2,  // 礼物赠送记录详情id，可用于答谢
            userId: 4, 
            greetting: "一厢情愿的不舍", // 祝福语
...
```js
{
  ...
  "data": {
    "id": 1,  
    "versionCode": “1”, //版本号
    "versionName": "礼遇圈1.0", //版本名称
    "title": "", //标题
    "type": 1,//1升级，2强制升级
    "apkUrl": "", 、、下载地址
    "upgradePoint": "版本更新", //更新说明
    "status": 1, //1正常
    "updated": 1555286995000, //更新时间
    "created": 1555286998000  //创建时间
  }
}
...
### 25.1 文章节日列表
- 请求地址：noauth/v1/article/festival_all
- 服务协议：HTTP/POST
- 是否需要身份认证：否
- 作者：linqin
  ...
  "data": [
    {
      "id": 1, //文章节日id
      "title": "元旦节", // 文章节日标题
      "created": 1561506273000, 
      "updated": 1561506276000
    },
    {
      "id": 2,
      "title": "除夕",
      "created": 1561506301000,
      "updated": 1561506306000
    }
  ]
}
```
...
### 25.2 文章场景列表
- 请求地址：noauth/v1/article/scene_all
- 服务协议：HTTP/POST
- 是否需要身份认证：否
- 作者：linqin
  ...
  "data": [
    {
      "id": 1, //文章场景id
      "title": "结婚",//文章场景标题
      "created": 1561506981000,
      "updated": 1561506986000
    },
    {
      "id": 2,
      "title": "乔迁",
      "created": 1561506999000,
      "updated": 1561507004000
    }
  ]
}
...
### 25.3 文章对象列表
- 请求地址：noauth/v1/article/label_all
- 服务协议：HTTP/POST
- 是否需要身份认证：否
- 作者：linqin
...
|   参数名称   | 参数类型 | 是否必传 | 默认值 | 参数说明 |
| :----------: | :------: | :------: | :----: | :------: |
| sceneId|int|否|无|文章场景id, 0 代表查询所有场景的文章|
| labelId|int|否|无|文章对象id, 0 代表查询所有对象的文章|
| festivalId|int|否|无|文章节日id, 0 代表查询所有节日的文章|
| keywords|string|否|无|关键字|
| pageNum | int | 是 | 1 | 分页 |
| pageSize | int | 是 | 14 | 分页大小 |
...
```js
{
  ...
  "data": [
    {
      "id": 190, //文章id
      "title": "一花一世界，一酒一人生", // 文章标题
      "summary": "没有白酒的激烈，也没有啤酒的寡淡，却带着玫瑰与酒融合后独有的芬芳，入口醇香，回味无穷。", //文章简介
      "cover": "https://liyuquan.cn/static/item/190531/bcdfb54d-0297-4b18-9037-6428ec906558.jpg" //文章封面
    },
    {
      "id": 194,
      "title": "鲜花饼助攻绿色生活，为环保吃出贡献！",
      "summary": "爱买鲜花饼这种类型的绿色食品，这样的生活既健康又绿色！",
      "cover": "https://liyuquan.cn/static/item/190603/1e98c4ce-af62-4a59-87ee-af24d9b7fa3a.png"
    }
  ]
}
...
```js
{
  ...
  "data": {
    ...
    "laterMoney": 0 //扣款后金额
    "cardType": 11, //会员卡类型 1 礼遇圈卡 10 普通卡 11 活动卡
    "capital": 90.00, //充值本金消费金额
    "send": 10.00//赠送金额消费金额
  }
}
...
- 请求地址：auth/v3/memberCard/update_password
- 服务协议：HTTP/POST
- 是否需要身份认证：是
- 作者：linqin
...
|oldPassword| String|是|无|旧密码（MD5加密）|
|newPassword|string|是|无|新密码（MD5加密）|
...
- 请求地址：auth/v3/memberCard/find_password
- 服务协议：HTTP/POST
- 是否需要身份认证：是
- 作者：linqin
...
```js
{
  ...
  "data": { //设计图的字段
    ...
    "sendMoney": 4.00, //转赠金额
    "summary": "已接收", //简介
    "status": 2, //0 超过24小时，已退回 1 未接收（可以领取） 2 已接收 3 自己转赠的 4 被别人接收（一般不会有情况4） 
  ...
  "data": [
    {
      ...
      "quantity": 5,//数量
      "code": "", // 二维码
      "created": 1581315411000, //优惠券领取时间
      "title": "20元优惠券",//标题
      "summary": "使用说明",//使用说明
      "logo": "https://liyuquan.cn/static", //优惠券图片
      "storeIds": "3,4", //适用店铺id 
      "date": 1586499358000, //优惠券有效期截止时间
      "startTime": 1582686153000, // 优惠券有效期开始时间
      "storeName": "外婆味道南屏店/外婆味道吾悦店" //适用店铺
      "status": 3 // 1 正常 2 未开始 3 已结束
    }
  ]
}
...
```js
{
  ...
  "data": {
    ...
    "title": "20元优惠券", //标题
    "summary": "使用说明", //使用说明
    "logo": "https://liyuquan.cn/static",//图片
    "storeIds": "3,4", //适用店铺id
    "date": 1586499358000, //有效期
    "storeName": "外婆味道南屏店/外婆味道吾悦店", //适用店铺名称
    "status": 1 // 0 超过24小时，已撤回  1 可以领取 2 已领取 3 被别人领取，自己无法领取 4 对方已接收  5 等待对方领取（ 4,5 赠送者自己点击的情况）
  }
}
